...
(function () {
  ...
  /* ---------- PublicKey validator (pinpoints bad inputs) ---------- */
  function safePk(label, input) {
    try {
      const val = (typeof input === 'string') ? input.trim() : input;
      const pk = (val instanceof solanaWeb3.PublicKey) ? val : new solanaWeb3.PublicKey(val);
      console.log(`[pk OK] ${label}:`, pk.toBase58());
      return pk;
    } catch (e) {
      console.error(`[pk BAD] ${label}:`, input, e?.message || e);
      throw e;
    }
  }
  ...
  const PROGRAM_ID = safePk('PROGRAM_ID', "GKiKsPmSQHGvg5VFXAGy99vmb3JV9BPnqFzC9iwp95Km");
  const MINT       = safePk('MINT',       "9UneRk37ZQppx2wBjekUVaSDVL789MdBMxy8YiRVx848");
  ...
  const ataFor = (owner) => {
    const TOKEN_PROGRAM_ID_CANON = safePk('TOKEN_PROGRAM_ID_CANON', "TokenkegQfeZyiNwAJbNbGKPFXCWuBvf9Ss623VQ5DA");
    const ATA_PROGRAM_ID_CANON   = safePk('ATA_PROGRAM_ID_CANON',   "ATokenGPvR93gBfue3DBeQ8Z8CwRk3s8H7RkG4GZLFpR");
